...
const Admin = ({ isType, unauthentication }) => {
  ...
  if (isType !== "a") return <Redirect to="/authentication" />;

  return (
    <div>
      <div className="header d-flex">
        <ul className="d-block w-100">
          <Link to="/admin/patients" className="btn mr-2">
            Пациенты
          </Link>
          <Link to="/admin/doctors" className="btn mr-2">
            Работники
          </Link>
          <Link to="/admin/journal" className="btn mr-2">
            Журнал
          </Link>
          <Link to="/admin/diagnoses" className="btn mr-2">
            Диагнозы
          </Link>
          <Link to="/admin/anaesthesia" className="btn">
            Анестезия
          </Link>
          <button onClick={onLogOutBtn} className="btn btn-primary exitbtn">
            Выход
          </button>
        </ul>
      </div>
