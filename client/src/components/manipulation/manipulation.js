...
const Manipulation = ({
  ...
  const onOkBtnClick = () => {
    const data = {
      query:
        "UPDATE `journal` SET `state` = ?,`is_deciduous` = ?,`note` = ?,`date_done` = ? WHERE `id` = ? ;",
      params: [
        journState,
        isDeciduous,
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Manipulation)
);
