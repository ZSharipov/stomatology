...
const Doctors = ({
  defaultHiddenColumnNames = ["date_created", "date_edit"],
  doctors,
  fetchDoctors,
}) => {
  ...
  const commitChanges = (args) => {
    ...
    if (added) {
      ...
      postDoctors({
        fio: added[0].fio,
        station: added[0].station,
        tel: added[0].tel,
        authentication: added[0].authentication,
        isType: added[0].isType,
      })
        .then((res) => res.json())
        .then((res) => {
          fetchDoctors();
    ...
    if (changed) {
      ...
      if (editRow) {
        const data = {
          query:
            "UPDATE `doctors` SET `fio` = ?, `station`=?, `tel` = ?,`authentication` = ?,`isType` = ? WHERE `id` = ?",
          params: [
            editRow.fio,
            editRow.station,
            editRow.tel,
            editRow.authentication,
            editRow.isType,
            editRow.id,
          ],
        };
  ...
  return (
    <div>
      <Grid rows={rows} columns={columns} getRowId={getRowId}>
        ...
        />
        <PagingPanel pageSizes={[5, 10, 100]} messages={pagingPanelMessages} />
      </Grid>
    </div>
  );
};
